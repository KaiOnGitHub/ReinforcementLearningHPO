...
def run_ppo(learning_rate: float, gamma: float, clip: float, environment: str = 'CartPole-v1'):
    seed = 67890
    # Create log dir
    log_dir = "tmp%s_%s_%s_%s/" % (environment, learning_rate, gamma, clip)
    ...
    with open("%s_PPO_random_lr_%s_gamma_%s_clip_%s_seed%s_eval.json" % (environment, learning_rate, gamma, clip, seed),
              'w+') as f:
        json.dump(data, f)
...
for lr, gamma, clip in zip(learning_rates, gammas, clips):
    r, std_r = run_ppo(learning_rate=lr, gamma=gamma, clip=clip)
