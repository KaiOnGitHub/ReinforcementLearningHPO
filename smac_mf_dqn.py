...
def run_ppo(config: dict, environment: str = 'MountainCar-v0', policy: str = 'MlpPolicy'
    ...
    if os.path.exists(log_dir):
        ...
        model = DQN(policy, env, verbose=0, learning_rate=np.power(10, learning_rate), gamma=gamma,
                    ...
                    exploration_fraction=1, seed=seed)
        rewards = []
        std_rewards = []
...
if __name__ == "__main__":
    ...
    learning_rate = UniformFloatHyperparameter(
        "learning_rate", 0.000001, 0.01, default_value=0.001, log=True
    )
    gamma = UniformFloatHyperparameter(
        "gamma", 0.8, 1.0, default_value=0.99, log=False
    )
    clipping = UniformFloatHyperparameter(
        "clip", 0.005, 0.3, default_value=0.2, log=False
    )
